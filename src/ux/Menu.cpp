...
Menu::Menu(const std::shared_ptr<Window> window, GameRoundDrawer& gameDrawer, const std::shared_ptr<const Font>& font1, const std::shared_ptr<const Font>& font2)
	...
	, m_player1Text(font2, ACTOR_NAMES[m_actor1SelectionID], window, 0xFFFFFFFF)
	, m_player2Text(font2, ACTOR_NAMES[m_actor2SelectionID], window, 0xFFFFFFFF)
...
void Menu::onWindowResized(int width, int height) {
	...
	m_player1Text   .setHeightKeepAspect(height/10).rect.setPos(width/2-height/8  , height/2    );
	m_player2Text   .setHeightKeepAspect(height/10).rect.setPos(width/2+height/8  , height/2    );
...
void Menu::onLeftClick(int32_t x, int32_t y) {
	if (m_player1Text.rect.containsPoint(x, y)) {
		...
		m_player1Text.text = ACTOR_NAMES[m_actor1SelectionID];
		m_player1Text.update(m_window, true);
		m_player1Text.setHeightKeepAspect(m_player1Text.rect.getHeight());
	}
	else if (m_player2Text.rect.containsPoint(x, y)) {
		m_actor2SelectionID = (m_actor2SelectionID+1) % ACTOR_NAME_COUNT;
		m_player2Text.text = ACTOR_NAMES[m_actor2SelectionID];
		m_player2Text.update(m_window, true);
		m_player2Text.setHeightKeepAspect(m_player2Text.rect.getHeight());
	}
